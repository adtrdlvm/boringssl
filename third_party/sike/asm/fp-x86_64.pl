...
sub rdc {
  my $jump_redc_mulx=&alt_impl(".Lrdc_mulx_asm") if ($bmi2_adx);
  ...
  my $redc_mulx=&redc_common($mulx1, $mulx2, $mulx3, $mulx4) if ($bmi2_adx);
...
sub mul {
  my $jump_mul_mulx=&alt_impl(".Lmul_mulx") if ($bmi2_adx);
  my $mul_mulx=&mul_mulx() if ($bmi2_adx);
  my $mul_generic=&mul_generic();
